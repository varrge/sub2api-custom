...
const mocks = vi.hoisted(() => ({ replaceGroup: vi.fn(), getGroups: vi.fn(), showError: vi.fn(), showSuccess: vi.fn() }))
vi.mock('@/api/admin', () => ({ adminAPI: { users: { replaceGroup: mocks.replaceGroup, getAvailableGroups: mocks.getGroups } } }))
...
beforeEach(() => {
  vi.clearAllMocks()
  mocks.getGroups.mockResolvedValue([{ id: 2, name: 'New', status: 'active', is_exclusive: true, subscription_type: 'standard' }])
