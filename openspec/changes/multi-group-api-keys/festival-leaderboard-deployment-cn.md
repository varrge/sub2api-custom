...
用户要求将双节榜单作为系统功能，在设置中调整时间、标题等；UI 由 Kimi 实现，主代理完成配置存储、统计联动、审查修复与部署。

- 配置入口：系统设置 → 消费榜，独立保存开关、标题、副标题、奖励说明、开始/结束时间和可选演示截止时间。北京时间输入，保留非零秒；关闭后隐藏奖杯。修改活动时间后旧缓存和旧请求不会覆盖新榜。
- 新接口：管理员 `GET/PUT /api/v1/admin/settings/activity-leaderboard`；用户 `GET /api/v1/activities/leaderboard/config` 和 `/api/v1/activities/leaderboard`，兼容旧 URL。配置存储于单条 settings JSON，无数据库迁移。
- 最终提交：`44f867480b54ca9c10f9db87213cf8c04afb758b`；版本：`0.2.7-custom.7-candidate.leaderboard-config.44f867480`。
- 镜像 ID：`sha256:dd825a62a8dc071bff57c5d704bea9c6c01cfda8b994d4d5ef3fc584b3d2dd7d`；二进制 SHA-256：`dad8688046b5dbfe034bb2db81325b9d291ffcb6c385ef6273112b96f0a5a07f`。
- 备份/证据目录：`/home/yinan/sub2api-staging/leaderboard-config-20260922T134744Z`；回滚镜像 `sub2api-restore:leaderboard-config-20260922t134744z`。首次配置版本证据保存在同前缀 `20260922T115614Z` 目录。
- 前端全量检查其余 336 个文件通过，最终相关 6 个文件/47 项通过；父设置页46项通过。窄屏后续7项复测、ESLint、生产类型编译与构建通过。后端五包全量、race、PostgreSQL16 SQL 测试通过，后端及前端独立审查问题已修复。
- 中国实例 API 实测：匿名401，普通用户可看且不能保存；完整保存/读取、非法配置不覆盖、禁用空榜、活动时间改变重新统计、旧路径兼容均通过。
- 中国浏览器：管理员保存标题后浮层更新，开关关闭/恢复、非法时间拦截、输入框 Enter 不提交其他设置、美国浏览器时区转换、375/320px设置页不溢出均通过。普通用户1440/1024/375/320px、明暗主题、矮屏浮层和关闭交互通过，无页面错误。
- 最终已恢复标题“中秋国庆双节消费榜”、2026-09-25 00:00至2026-10-08 00:00、启用状态及原演示期限 `2026-09-23T19:23:36+08:00`。演示环境变量移除，改用可编辑系统设置。
- 仅重建中国测试应用，健康检查正常、零重启；其他容器及 users154/151、accounts57/19、keys225/158、subscriptions49/24记录计数不变。
- 清理本功能旧部署的可重建解压目录，归档、备份和镜像保留；未操作日本生产，未合并、推送或打 tag。
- 隧道：<http://127.0.0.1:18081/admin/settings>，用户查看榜单：<http://127.0.0.1:18081/keys>。
