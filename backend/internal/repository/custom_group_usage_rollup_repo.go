...
func (r *usageLogRepository) getAllGroupUsageSummaryFromRollups(ctx context.Context, todayStart time.Time) (results []usagestats.GroupUsageSummary, err error) {
	// The watermark and totals must share a PostgreSQL snapshot. Otherwise a
	// cleanup or late usage write between the two reads can invalidate a bucket
	// while the totals still trust its old watermark. Keep the tail bound as a
	// parameter so PostgreSQL can continue using the created_at index.
	if beginner, ok := r.sql.(interface {
		...
		results, err := txRepo.getAllGroupUsageSummaryInSnapshot(ctx, todayStart)
		if err != nil {
			return nil, err
		}
	...
	// An existing transaction retains its caller-owned isolation and lifecycle.
	return r.getAllGroupUsageSummaryInSnapshot(ctx, todayStart)
}

func (r *usageLogRepository) getAllGroupUsageSummaryInSnapshot(ctx context.Context, todayStart time.Time) (results []usagestats.GroupUsageSummary, err error) {
