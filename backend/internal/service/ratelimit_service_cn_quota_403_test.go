...
func TestKimiQuota403ConditionalProbeObservesOnlyAccountRateLimit(t *testing.T) {
	for _, resetIn := range []time.Duration{0, time.Hour} {
		repo := &rateLimitAccountRepoStub{}
		svc := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
		account := newCNQuotaExhaustedTestAccount(resetIn)
		account.Status, account.Schedulable = StatusActive, true
		require.True(t, runKimi403(svc, account, kimiWeeklyQuotaExhaustedBody))
		if repo.rateLimitedCalls > 0 {
			account.RateLimitResetAt = &repo.lastRateLimitedAt
		} else {
			until := time.Now().Add(time.Hour)
			account.TempUnschedulableUntil = &until
		}
		snapshot := scheduledModelLimitSnapshot(t.Context(), account, "kimi-for-coding", time.Now())
		require.Equal(t, resetIn > 0, snapshot.limited(), "the fallback temp-unschedulable state must not expand conditional-probe triggers")
	}
}
