...
import { useBatchImageAccess, useImageGenerationAccess } from '@/composables/useBatchImageAccess'
...
const { canUseImageGeneration } = useImageGenerationAccess()
    ...
    h(
      ...
      [
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M6.75 3.75h10.5a3 3 0 013 3v10.5a3 3 0 01-3 3H6.75a3 3 0 01-3-3V6.75a3 3 0 013-3zM3.75 15l4.19-4.19a1.5 1.5 0 012.12 0L18.75 19.5M15.75 9h.008v.008h-.008V9z'
        }),
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M18.75 2.25v3m1.5-1.5h-3'
        })
      ]
    )
}
...
const flagImageGenerationAccess = () => flagImageGeneration() && canUseImageGeneration.value
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/image-generation', label: t('nav.imageGeneration'), icon: ImageGenerationIcon, hideInSimpleMode: true, featureFlag: flagImageGenerationAccess },
    { path: '/batch-image', label: t('nav.batchImage'), icon: BatchImageIcon, hideInSimpleMode: true, featureFlag: flagBatchImageAccess },
    { path: '/usage', label: t('nav.usage'), icon: ChartIcon, hideInSimpleMode: true },
    { path: '/available-channels', label: t('nav.availableChannels'), icon: ChannelIcon, hideInSimpleMode: true, featureFlag: flagAvailableChannels },
    { path: '/monitor', label: t('nav.channelStatus'), icon: SignalIcon, featureFlag: flagChannelMonitor },
    { path: '/group-buy', label: t('groupBuy.hall'), icon: UsersIcon, hideInSimpleMode: true, featureFlag: flagPayment },
...
const adminPersonalNavItems = computed((): NavItem[] => (
  authStore.isSimpleMode
    ? personalNavItems.value.filter((item) => item.path === '/tickets')
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/group-buy', label: t('groupBuy.admin'), icon: UsersIcon, hideInSimpleMode: true, featureFlag: flagAdminPayment },
    { path: '/admin/subscriptions', label: t('nav.subscriptions'), icon: CreditCardIcon, hideInSimpleMode: true },
    { path: '/admin/accounts', label: t('nav.accounts'), icon: GlobeIcon },
    { path: '/admin/plugins', label: t('nav.plugins'), icon: PluginIcon, featureFlag: flagPluginManagement },
    { path: '/admin/announcements', label: t('nav.announcements'), icon: BellIcon },
    { path: '/admin/tickets', label: t('supportTickets.management'), icon: TicketIcon, badge: () => supportTicketStore.adminUnreadCount },
