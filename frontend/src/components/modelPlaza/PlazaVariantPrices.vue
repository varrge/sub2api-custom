<template>
  <section :data-price-group="variant.group.id" :data-price-platform="variant.model.platform">
    <header class="flex flex-wrap items-center justify-between gap-2 border-t border-gray-100 bg-gray-50/60 px-4 py-3 dark:border-dark-700 dark:bg-dark-900/30">
      ...
      <span v-for="[effort, multiplier] in reasoningMultipliers" :key="effort" class="text-xs text-amber-700 dark:text-amber-300" :data-reasoning-effort="effort" :title="t('modelPlaza.table.reasoningMultiplierHint', { effort, multiplier })">{{ t('modelPlaza.table.reasoningMultiplierBadge', { effort, multiplier }) }}</span>
...
import { type GroupModelVariant, type PlazaPriceCell, modelPriceCells, formatUsdDirect, formatTokenLimit, reasoningEffortMultipliers } from './plaza-models'
...
const threshold = computed(() => props.variant.model.pricing?.billing_mode === 'token'
  ...
  : undefined)
const reasoningMultipliers = computed(() => reasoningEffortMultipliers(props.variant.model))
const hasSpecialPricing = computed(() => !!threshold.value || !!props.variant.model.time_pricing?.periods.length || props.variant.group.peak_rate_enabled || reasoningMultipliers.value.length > 0)
