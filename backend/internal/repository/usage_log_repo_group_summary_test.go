...
func TestUsageLogRepositoryGetAllGroupUsageSummaryRollsBackOnReadFailure(t *testing.T) {
	db, mock := newSQLMock(t)
	repo := newUsageLogRepositoryWithSQL(nil, db)
	readErr := errors.New("usage summary read failed")
	mock.ExpectBegin()
	mock.ExpectQuery(`(?s)COUNT\(\*\).*usage_group_rollup_state`).WillReturnError(readErr)
	mock.ExpectRollback()
