...
浮层打开后才请求，打开期间每分钟刷新，后台标签页暂停定时请求；关闭/卸载会取消请求并清理定时器。刷新失败保留上次数据并提示，首次失败提供重试。日期始终按北京时间展示，中英文、明暗主题均支持。
...
用户要求临时填充几条数据用于查看下拉榜单。服务端提供默认关闭的 `ACTIVITY_LEADERBOARD_DEMO_UNTIL`（RFC3339 绝对时间）开关，仅在活动开始前且未到期时返回 8 条独立演示数据；不会写数据库或混入真实缓存。任何用户看到的“我的排名”示例均为第 4 名，界面明确说明所有金额和名次均为模拟，并显示到期时间。

本次仅在中国测试实例设为 `2026-09-23T19:23:36+08:00`。时间到自动恢复真实榜单；即使配置了更晚期限，9 月 25 日活动开始时也强制禁用演示。提前关闭可移除 Compose 中的该环境变量并仅重建应用，或者回滚到此次预览前的镜像。

验证覆盖：默认关闭、无效配置关闭、演示 8 行/第 4 名、到期自动回到空榜、活动开始强制采用真实数据，以及前端从演示恢复真实状态。
