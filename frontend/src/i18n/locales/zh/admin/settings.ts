export default {
    settings: {
      ...
      activityLeaderboard: {
        title: '消费榜活动',
        description: '配置用户端顶栏奖杯入口展示的消费榜活动。此配置独立于其他系统设置，使用本卡片的保存按钮单独保存。',
        enabled: '启用消费榜',
        enabledHint: '关闭后隐藏奖杯入口；已打开的其他页面最迟约一分钟内同步。',
        fieldTitle: '活动标题',
        fieldTitleHint: '必填，1–80 个字符，展示在榜单弹层顶部。',
        fieldSubtitle: '副标题',
        fieldSubtitleHint: '可选，最多 160 个字符；留空则榜单内不展示该行。',
        fieldReward: '奖励说明',
        fieldRewardHint: '可选，最多 1000 个字符；留空则榜单内不展示该行。',
        startsAt: '开始时间',
        endsAt: '结束时间',
        timeHint: '按北京时间（UTC+08:00）填写，与浏览器时区无关。统计含开始、不含结束。',
        demoExpiresAt: '演示数据到期时间',
        demoExpiresAtClear: '清除',
        demoExpiresAtHint: '可选，且不能晚于开始时间；仅在活动未开始前展示模拟榜单，到期或活动正式开始后自动停止，演示不产生真实计费、不写入数据库。',
        timeZoneNote: '所有时间均按北京时间（UTC+08:00）填写与展示。',
        billingNote: '榜单统计的是计费额度，不是人民币实付金额。',
        rulesNote: '统计范围：所有分组的余额、订阅及拼团月卡在优惠与专属价生效后的实际计费消耗均计入；充值与购买套餐不计入。榜单展示前 20 名与本人排名，用户以匿名编号展示。',
        loadFailed: '消费榜配置加载失败，请重试。',
        retry: '重试',
        saved: '消费榜配置已保存',
        saveFailed: '保存消费榜配置失败',
        validation: {
          title: '活动标题必填，且不超过 80 个字符。',
          subtitle: '副标题不能超过 160 个字符。',
          rewardDescription: '奖励说明不能超过 1000 个字符。',
          periodRequired: '请填写完整且有效的开始与结束时间。',
          periodOrder: '开始时间必须早于结束时间。',
          periodLength: '活动时长不能超过 366 天。',
          demoInvalid: '演示数据到期时间无效。',
          demoAfterStart: '演示数据到期时间不能晚于活动开始时间。',
        },
      },
      panelRateLimit: {
        title: '面板接口限流',
