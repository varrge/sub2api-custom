<template>
  <article
    class="group relative flex flex-col justify-between overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary-400/60 hover:shadow-lg dark:border-dark-700/80 dark:bg-dark-800 dark:hover:border-primary-500/50"
  >
    <!-- Top accent bar -->
    <div class="h-1 w-full bg-gradient-to-r from-primary-500 via-primary-300 to-primary-400 dark:from-primary-400 dark:via-primary-400 dark:to-primary-500" />
    ...
    <div class="flex flex-1 flex-col p-5 sm:p-6">
      ...
      <div class="flex items-center justify-between gap-2">
        ...
        <span
          ...
          class="inline-flex shrink-0 items-center gap-1 rounded-md border border-primary-100 bg-primary-50/80 px-2.5 py-1 text-xs font-medium text-primary-700 dark:border-primary-900/60 dark:bg-primary-950/50 dark:text-primary-300"
        >
      ...
      <div class="mt-3">
        <h3
          class="text-lg sm:text-xl font-bold tracking-tight text-slate-900 dark:text-on-primary break-words"
          :title="product.name"
        >
          {{ product.name }}
        </h3>
      ...
      <div class="mt-4 flex flex-wrap items-baseline gap-1">
        <span class="min-w-0 text-3xl font-extrabold tracking-tight text-slate-900 [overflow-wrap:anywhere] dark:text-on-primary">
      ...
      <div class="mt-4 grid grid-cols-[repeat(auto-fit,minmax(min(100%,8rem),1fr))] gap-3 rounded-xl border border-slate-100 bg-slate-50/80 p-3 text-xs dark:border-dark-700/60 dark:bg-dark-700/40">
        <div class="min-w-0">
          ...
          <span class="mt-0.5 block text-base font-bold text-primary-700 [overflow-wrap:anywhere] dark:text-primary-300">
        ...
        <div class="space-y-1.5">
          ...
          <div
            ...
            :class="[
              ...
              isHighestTier(tier)
                ? 'border border-primary-200/80 bg-primary-50/60 text-primary-950 dark:border-primary-800/60 dark:bg-primary-950/40 dark:text-primary-200'
          ...
          >
            <div class="flex flex-wrap items-center gap-1.5">
              <span
                :class="[
                  ...
                  isHighestTier(tier) ? 'bg-primary-500 dark:bg-primary-400' : 'bg-slate-300 dark:bg-dark-500'
                ]"
              />
              ...
              <span
                ...
                class="rounded bg-primary-600 px-1.5 py-0.5 text-[10px] font-semibold text-on-primary dark:bg-primary-500 dark:text-dark-900"
              >
            ...
            <div class="min-w-0 font-semibold [overflow-wrap:anywhere]">
              <span :class="isHighestTier(tier) ? 'text-primary-700 dark:text-primary-300 font-bold' : 'text-slate-800 dark:text-slate-200'">
                {{ usd(tier.quota_usd) }}
              </span>
              <span class="font-normal text-[11px] text-slate-400 dark:text-dark-400"> {{ t('groupBuy.perCardSuffix') }}</span>
            </div>
      ...
      <div class="mt-5 grid grid-cols-2 gap-3">
        ...
        <button
          type="button"
          class="flex flex-col items-center justify-center rounded-xl bg-primary-600 py-2.5 px-3 text-center text-on-primary shadow-sm transition-all hover:bg-primary-500 active:bg-primary-700 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-dark-900"
          @click="emit('select', { product, mode: 'create' })"
        >
          <span class="flex items-center gap-1 text-sm font-bold text-on-primary">
            <svg aria-hidden="true" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
              <path d="M13 6a3 3 0 11-6 0 3 3 0 016 0zM18 8a2 2 0 11-4 0 2 2 0 014 0zM14 15a4 4 0 00-8 0v3h8v-3zM6 8a2 2 0 11-4 0 2 2 0 014 0zM16 18v-3a5.972 5.972 0 00-.75-2.906A3.005 3.005 0 0119 15v3h-3zM4.75 12.094A5.973 5.973 0 004 15v3H1v-3a3 3 0 013.75-2.906z" />
            </svg>
            {{ t('groupBuy.create') }}
          </span>
          <span class="mt-0.5 text-[11px] text-on-primary/80">
