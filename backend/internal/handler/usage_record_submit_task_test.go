...
func TestOpenAIGatewayHandlerSubmitOpenAIUsageRecordTask_SeedanceUsesMandatoryFallback(t *testing.T) {
	...
	var called atomic.Bool
	h.submitOpenAIUsageRecordTask(context.Background(), &service.OpenAIForwardResult{ResponseID: "seedance:task", Usage: service.OpenAIUsage{OutputTokens: 1000}}, func(ctx context.Context) {
		called.Store(true)
	})
	close(release)

	require.True(t, called.Load(), "Seedance usage task must be mandatory when async submit is dropped")
}
