...
describe('catalog selection and interactions', () => {
  ...
  it('keeps composite platform prices separate even when only one reference group is selected', () => {
    const entry = aggregatePlazaModels([group({ platform: 'composite', models: [model(), model({ platform: 'antigravity', pricing: { ...model().pricing!, input_price: 8e-6 } })] })])[0]
    const wrapper = mount(PlazaModelCard, { props: { model: entry, priceGroupId: 1, priceGroupIds: [1] } })
    expect(wrapper.findAll('[data-price-group]')).toHaveLength(2)
    expect(wrapper.get('[data-price-platform="anthropic"] [data-price="input_price"]').text()).toBe('$1')
    expect(wrapper.get('[data-price-platform="antigravity"] [data-price="input_price"]').text()).toBe('$4')
    wrapper.unmount()
  })
