export default {
  ...
  keys: {
    ...
    multiGroup: {
      ...
      moveUp: "提高「{name}」的优先级",
      moveDown: "降低「{name}」的优先级",
      remove: "移除「{name}」",
      add: "添加「{name}」",
      adminEmpty: "未选择分组。保存将解绑全部分组；未分组密钥是否允许调度取决于服务器设置。",
      inactive: "分组已停用。绑定与顺序仍保留，恢复可用后自动参与选组。",
      ineligible: "该用户目前没有此分组的使用资格（例如订阅已过期或权限被撤销）。绑定仍保留，恢复资格后自动参与选组。",
      subscriptionCharge: "使用此分组的订阅或月卡权益，倍率见上方。",
      balanceCharge: "使用账户余额，或此分组适用的月卡权益，倍率见上方。",
      priceWarning: "前序分组不可用时，可能选中已选列表中价格更高或使用付费余额的分组。每次请求按实际分组的价格和权益结算；不同分组不共享月卡额度。",
      strictScope: "已启用多分组模式：选组和兜底始终限定在已选且有资格的分组内，缩回单组也不会恢复旧兜底权限。",
      enableScope: "选择多个分组后，选组和兜底将永久限定在该密钥已选且有资格的分组内。以后缩回单组也不会恢复旧兜底权限。",
      sharedQuota: "密钥总额度及 5 小时 / 1 天 / 7 天消费限额会累计所有已选分组的用量。",
      loadFailed: "无法加载该用户已有资格的分组。",
      replaceHint: "只能选择该用户已有资格的分组。替换时保留其他绑定及顺序并去重，不会自动授予分组权限。",
    },
    title: 'API 密钥',
  ...
  modelPlaza: {
    catalog: {
      ...
      "group": "分组",
      "allGroups": "全部分组",
      "referenceSelectAll": "全选",
      "referenceClear": "清空",
      "referenceSelected": "已选 {count}/{total} 个分组",
      "noReferenceSelected": "未选择价格参考分组",
      "chooseReferenceGroups": "请勾选价格参考分组，或点击全选查看价格。",
      "referenceGroup": "价格参考分组",
      "referenceHint": "可多选或全选，同时对比各分组价格，不改变模型范围。",
      "noGroups": "暂无分组",
      "suppliers": "供应商",
      "allSuppliers": "全部供应商",
      "reset": "重置筛选",
      ...
      "perMillion": "/M",
      "original": "基础价",
      "unavailableInReference": "所选参考分组暂无此模型，请调整价格参考分组。",
      "groupPrices": "分组价格",
    ...
    selectGroupsHint: '请选择分组，或点击「全选」查看所有分组的价格',
    filters: {
      selectedGroups: '已选 {selected}/{total} 个分组',
      multiSelectHint: '可多选；全选显示当前平台和倍率下的所有分组',
      clearSearch: '清空搜索',
      clearGroups: '清空',
      selectAll: '全选',
      platformLabel: '平台',
