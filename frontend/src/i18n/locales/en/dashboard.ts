export default {
  ...
  keys: {
    ...
    multiGroup: {
      ...
      moveDown: "Decrease priority for {name}",
      remove: "Remove {name}",
      add: "Add {name}",
      adminEmpty: "No groups selected. Saving will unbind every group; ungrouped scheduling follows the server setting.",
      inactive: "Group disabled. This binding and priority are kept; it participates again when available.",
      ineligible: "This user is currently ineligible for this group (for example, an expired subscription or removed access). The binding is kept and resumes automatically when eligibility returns.",
      subscriptionCharge: "Uses this group’s subscription or month card entitlement; rates shown above.",
      balanceCharge: "Charges account balance, or uses an applicable month card for this group; rates shown above.",
      priceWarning: "An unavailable group may be skipped for a selected group with a higher price or paid balance billing. Each request uses the chosen group’s prices and entitlements; month card quotas are not shared across groups.",
      strictScope: "Multi-group mode is enabled: all selection and fallback stay within the selected eligible groups, even after reducing this key to one group.",
      enableScope: "Selecting multiple groups permanently limits selection and fallback to this key’s selected eligible groups. Returning to one group does not restore legacy fallback access.",
      sharedQuota: "The key’s total quota and 5h / 1d / 7d spending limits accumulate usage from all selected groups.",
      loadFailed: "Could not load this user’s eligible groups.",
      replaceHint: "Only groups this user is already eligible for can be selected. Replacing a group preserves other bindings and their order, and removes duplicates without granting access.",
    },
    title: 'API Keys',
  ...
  modelPlaza: {
    catalog: {
      ...
      "group": "Group",
      "allGroups": "All groups",
      "referenceSelectAll": "Select all",
      "referenceClear": "Clear",
      "referenceSelected": "{count}/{total} groups selected",
      "noReferenceSelected": "No price reference groups selected",
      "chooseReferenceGroups": "Choose price reference groups, or select all to view prices.",
      "referenceGroup": "Price reference group",
      "referenceHint": "Select multiple groups or all to compare prices without narrowing the model catalog.",
      ...
      "perMillion": "/M",
      "original": "Base price",
      "unavailableInReference": "Not available in the selected reference groups. Choose another price reference group.",
