...
describe('bulk default prices', () => {
  it('preserves configured reasoning pricing overrides and imports all catalog levels', () => {
    const entry = blank(['claude-fable-5-1'], { reasoning_effort_multipliers: { high: 1.5 } })
    ...
    const result = mergeDefaultPricing([], [{ model: 'claude-fable-5-1', pricing: {
      found: true, input_price: 10e-6, reasoning_effort_multipliers: { high: 1.5, max: 3 }
    } }])
    expect(result.entries[0].reasoning_effort_multipliers).toEqual({ high: 1.5, max: 3 })
    expect(canImportDefaultPricing('claude-fable-5-1', [blank(['claude-fable-5-1'], { reasoning_effort_multipliers: { high: '' } })])).toBe(true)
  })
