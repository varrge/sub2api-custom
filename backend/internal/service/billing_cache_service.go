...
// CheckBillingEligibilityReadOnly verifies balance, entitlement and spending
...
func (s *BillingCacheService) CheckBillingEligibilityReadOnly(ctx context.Context, user *User, apiKey *APIKey, group *Group, subscription *UserSubscription, platform string) error {
	...
	if s.monthCardStore != nil {
		if err := s.monthCardStore.CheckDebt(ctx, user.ID); err != nil {
			return mapMonthCardAdmissionError(err)
		}
	}
...
func (s *BillingCacheService) checkSubscriptionEligibility(ctx context.Context, userID int64, group *Group, subscription *UserSubscription) error {
	if snapshot := subscription.MonthCardSnapshot; snapshot != nil {
		if snapshot.UserID != userID || snapshot.GroupID != group.ID || len(snapshot.Candidates) == 0 {
			return ErrSubscriptionInvalid
		}
		return nil
	}
