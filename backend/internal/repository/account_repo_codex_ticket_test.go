...
func TestLockAndMergeAccountExtraPreservesTicketsAlongsideOpenCodeState(t *testing.T) {
	...
	account.Extra = map[string]any{
		...
		"codex_turn_ticket:model":                  map[string]any{"state": "stale"},
		"codex_turn_ticket:injected":               map[string]any{"state": "spoofed"},
	}
	mock.ExpectQuery(`(?s)SELECT.*FOR NO KEY UPDATE`).
		WillReturnRows(sqlmock.NewRows(openCodeGoMergeMockColumns()).AddRow(
			true, false, true, nil, nil, nil, nil, nil, nil,
			true, []byte(`true`), []byte(`{"status":"ok"}`),
			[]byte(`{"codex_turn_ticket:model":{"state":"latest-database-ticket"}}`),
		))
	extra, err := lockAndMergeAccountProbeExtra(context.Background(), client, account, nil, nil)
	require.NoError(t, err)
	require.Equal(t, true, extra[service.OpenCodeGoUsageAutoRefreshExtraKey])
	require.Equal(t, map[string]any{"status": "ok"}, extra[service.OpenCodeGoUsageSnapshotExtraKey])
	require.Equal(t, map[string]any{"state": "latest-database-ticket"}, extra["codex_turn_ticket:model"])
	require.NotContains(t, extra, "codex_turn_ticket:injected")
	require.NoError(t, mock.ExpectationsWereMet())
}
